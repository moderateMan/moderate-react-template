...
export default class FetchRequest {
   ...
   * @param params    请求参数
   */
  static post(url: string, requestData = "", options = {}) {
    const { global } = stores;
    // global.changeParams({ isLoading: true });
    /**定义token变量*/
    let token: any = ""; //用户登入的token
    /**
    ...
    let timer: any = "";
    if (timeout) {
      const timeoutFn: Promise<any> = new Promise((resolve, reject) => {
        timer = setTimeout(() => reject(this.timeoutFlag), timeout * 1000);
      });
  ...
  static requestPostBlob(url: string, requestData = "", options: any = {}) {
    let token: any = "";
    if (!this.unAuthorizateUrls.includes(url)) {
      token = Storage.getStorage(ACCESS_TOKEN);
      if (!token) {
  ...
  static requestPostFileBlob(url: string, requestData = "", options: any = {}) {
    let token: any = "";
    if (!this.unAuthorizateUrls.includes(url)) {
      token = Storage.getStorage(ACCESS_TOKEN);
      if (!token) {
